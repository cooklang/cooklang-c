...
# COOKLANG's versioning is <MAJOR>.<MINOR>.<PATCH>[-DEVEL]
...
TEST_BINS = \
		$(BUILDDIR)/test/units/cooklang-shared \
		$(BUILDDIR)/test/units/cooklang-static

all: $(BUILDDIR)/$(LIB_SH_MAJ) $(BUILDDIR)/$(LIB_STATIC)
...
$(BUILDDIR)/test/units/cooklang-static: $(TEST_OBJ) $(BUILDDIR)/$(LIB_STATIC)
	$(CC) $(LDFLAGS_COV) -o $@ $^ $(LDFLAGS)

$(BUILDDIR)/test/units/cooklang-shared: $(TEST_OBJ) $(BUILDDIR)/$(LIB_SH_MAJ)
	$(CC) $(LDFLAGS_COV) -o $@ $^ $(LDFLAGS)
