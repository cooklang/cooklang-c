...
/** Clears the list: frees the contents of the list - Node structs and data
 *stored in them - without deleting the List struct uses the supplied function
 *pointer to release allocated memory for the data
 * @pre 'List' type must exist and be used in order to keep track of the linked
 *list.
 ...
 * @pre data should be of same size of void pointer on the users machine to
 *avoid size conflicts. data must be valid. data must be cast to void pointer
 *before being added.
 ...
 * @param data - is a void * pointer to any data type.  Data must be allocated
 *on the heap.
 **/
...
void* findElement(List* list,
  ...
  if (customCompare == NULL) return NULL;

  ListIterator itr = createIterator(list);

  void* data = nextElement(&itr);
  while (data != NULL) {
    if (customCompare(data, searchRecord)) return data;
