...
#define ASSERT_TOKEN_TYPE(tc, propesed_token, expected_token_type) do { \
    if (propesed_token.type != expected_token_type) { \
        return ttest_fail(&tc, "Line %d: expected %s, got %s", __LINE__, token_type_to_string(expected_token_type), token_type_to_string(propesed_token.type)); \
    } \
    } while(0)
...
static bool test_hello(ttest_report_ctx_t *report)
{
    ...
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOF);
...
static bool test_whitespace_only_input(ttest_report_ctx_t *report)
{
    ...
    init_lexer(L" ");
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOF);
...
static bool test_new_line_input(ttest_report_ctx_t *report)
{
    ...
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOL);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOF);
...
static bool test_string_with_numbers(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"abc 70770 xyz";
    init_lexer(input);

    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_INTEGER);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOF);
...
static bool test_string_with_decimal_numbers(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"abc 0.70770 xyz";
    init_lexer(input);

    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_DECIMAL);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOF);
...
static bool test_string_with_decimal_like_numbers(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"abc 01.70770 xyz";
    init_lexer(input);
...
static bool test_integer_like_numbers(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"70770hehe";
    init_lexer(input);
...
static bool test_fractional_numbers(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"1/2";
    init_lexer(input);

    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_FRACTIONAL);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOF);
...
static bool test_string_with_leading_zero_numbers(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"abc 0777 xyz";
    init_lexer(input);
...
static bool test_string_with_like_numbers(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"abc 7peppers xyz";
    init_lexer(input);
...
static bool test_string_with_punctuation(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"abc – xyz: lol,";
    init_lexer(input);
    ...
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_COLON);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOF);
...
static bool test_ingridients_one_liner(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"Add @onions{3%medium} chopped finely";
    init_lexer(input);

    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_AT);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_BRACES_LEFT);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_INTEGER);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_PERCENT);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_BRACES_RIGHT);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
...
static bool test_ingridients_decimal(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"@onions{3.5%medium}";
    init_lexer(input);
...
static bool test_ingridients_fractions(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"@onions{1/4%medium}";
    init_lexer(input);
...
static bool test_comments(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"-- testing comments";
    init_lexer(input);
...
static bool test_block_comments(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"visible [- hidden -] visible";
    init_lexer(input);
...
static bool test_block_comments_multiline(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"visible [- hidden"
                            "hidden"
                            "hidden -] visible";
...
static bool test_dash_in_text(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"Preheat the oven to 200℃-Fan 180°C.";
    init_lexer(input);
...
static bool test_ingridients_no_units(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"@onions{3}";
    init_lexer(input);
...
static bool test_ingridients_multi_word_no_amount(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"@red onions{}";
    init_lexer(input);
...
static bool test_ingridients_multi_word_with_punctuation(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"@onions, chopped";
    init_lexer(input);
...
static bool test_ingridients_word_no_amount(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"an @onion finely chopped";
    init_lexer(input);
...
static bool test_equipment(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"put into #oven";
    init_lexer(input);
    ...
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_HASH);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOF);
...
static bool test_equipment_multi_word(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"fry on #frying pan{}";
    init_lexer(input);
...
static bool test_timer(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L"cook for ~{10%minutes}";
    init_lexer(input);
    ...
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_TILDE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_BRACES_LEFT);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_INTEGER);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_PERCENT);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
...
static bool test_metadata(ttest_report_ctx_t *report)
{
    ...
    const wchar_t* input = L">> servings: 4|5|6";
    init_lexer(input);

    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_CHEVRON);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_CHEVRON);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WORD);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_COLON);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_WHITESPACE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_INTEGER);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_PIPE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_INTEGER);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_PIPE);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_INTEGER);
    ASSERT_TOKEN_TYPE(tc, next_token(), TOKEN_EOF);
